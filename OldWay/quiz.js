...
function submitQuiz() {
    const selectedQuizAnswers = correctAnswers[selectedQuiz]; // Obtenez les réponses correctes pour le quiz sélectionné
    const answers = [...document.querySelectorAll('input[type="radio"]:checked')];
    let score = 0;
    const testDate = getFormattedDate();

    answers.forEach((userAnswer, index) => {
        const questionIndex = userAnswer.name.replace('question', ''); // Extrait l'index de la question
        const correctAnswerKey = selectedQuizAnswers[index]; // Obtient la clé de la réponse correcte ('a', 'b', 'c', 'd')

        if (userAnswer.value === correctAnswerKey) {
            score++;
            userAnswer.closest('label').style.border = '2px solid green'; // Marque la réponse correcte
        } else {
            userAnswer.closest('label').style.border = '2px solid red'; // Marque la réponse incorrecte
        }
    });
    ...
    const FinalScore = score + "/" + quizzes[selectedQuiz].length;
    ...
    const url = "https://sheetdb.io/api/v1/xn7qlfy28tjy8"; // Votre URL d'endpoint SheetDB
